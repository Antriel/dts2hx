...
@:jsRequire("three", "ShapePath") extern class ShapePath {
	...
	**/
	...
	/**
		...
		of the shape, by default set to white _(0xffffff)_.
	**/
	var color : Color;
	/**
		Starts a new
	...
	**/
	...
	/**
		This creates a line from the
		{@link
		ShapePath.currentPath
		currentPath
		}
		's offset to X and Y and updates the offset to X and Y.
	**/
	function lineTo(x:Float, y:Float):ShapePath;
	/**
		This creates a quadratic curve from the
		{@link
		ShapePath.currentPath
		currentPath
		}
		's
		offset to _x_ and _y_ with _cpX_ and _cpY_ as control point and updates the
		{@link
		ShapePath.currentPath
		currentPath
		}
		's offset to _x_ and _y_.
	**/
	function quadraticCurveTo(aCPx:Float, aCPy:Float, aX:Float, aY:Float):ShapePath;
	/**
		This creates a bezier curve from the
		{@link
		ShapePath.currentPath
		currentPath
		}
